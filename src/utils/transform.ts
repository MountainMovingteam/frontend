...
export function getBookingData2Show(bookingData: any) {
	...
	for (let i = 0; i < bookingData.length; i++) {
		buf.push({
			...bookingData[i],
			status: bookingData[i].status == 1 ? '已驳回' : '未驳回',
			week_num: bookingData[i].week_num,
			commentator: bookingData[i].lecturers.length > 0 ? bookingData[i].lecturers[0] : '暂无',
			type: bookingData[i].type == 1 ? '团队预约' : '个人预约',
